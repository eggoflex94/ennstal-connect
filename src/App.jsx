...
useEffect(() => {
  ...
  async function saveProfile(e) {
    ...
    const layoutHours = { standard: 0, "theme-red": 0, "theme-blue": 0, "theme-neon": 0, "theme-alpine": 0, "theme-teal": 0, "theme-violet": 0, "theme-copper": 0, "theme-aurora": 0 };
...
function Profile({ profile, user, isHeadAdmin, saveProfile, uploadProfileImage, editProfileImage, uploadProfileBackground, editProfileCover, removeProfileCover, uploadProfileBioImage }) { const background = profile?.profile_background || "#1b1f26"; const isImage = background.startsWith("http"); const business = profile?.account_badge === "BUSINESS"; return <section>{profile?.is_community_photographer && <div className="community-photographer-self-banner panel"><img src="/community-photographer-camera.svg" alt=""/><div><span className="eyebrow">COMMUNITY-FOTOGRAF</span><strong>{profile?.community_photographer_global ? "Für alle Regionen freigeschaltet" : "Regional freigeschaltet"}</strong><small>Eventfotos kannst du im Bereich „Fotos“ hochladen und verwalten.</small></div></div>}<div className="my-area-layout"><div className="profile-member-card-preview" aria-label="Vorschau deiner Mitgliederkarte"><MemberCardView member={profile} profile={profile} friendships={[]} interactive={false}/></div><form className="panel profile-form profile-editor" onSubmit={saveProfile}>
  ...
  <fieldset className={`profile-editor-section layout-rewards ${business ? "business-layout-rewards" : ""}`}>
    ...
    <label className="profile-editor-field profile-editor-field-wide"><span>Profil-Layout</span><select name="profile_layout" defaultValue={["standard","theme-red","theme-blue","theme-neon","theme-alpine","theme-teal","theme-violet","theme-copper","theme-aurora"].includes(profile?.profile_layout) ? profile.profile_layout : "standard"}><option value="standard">Standard – Ennstal Connect</option><option value="theme-red">Connect Rot – Hellrot</option><option value="theme-alpine">Alpin Grün – Ruhig & Regional</option><option value="theme-blue">Connect Blau – Kräftig</option><option value="theme-teal">Bergsee Türkis – Frisch & Klar</option><option value="theme-violet">Enzian Violett – Modern & Edel</option><option value="theme-copper">Kupfer Nacht – Kupfer & Tiefpetrol</option><option value="theme-aurora">Polarlicht – Cyan, Magenta & Nachtblau</option><option value="theme-neon">Neon Grün – Giftgrün & Dunkel</option></select></label>
