...
export default function App() {
  ...
  async function joinGroup(group) { const { error } = await supabase.rpc("join_community_group", { p_group_id: group.id }); if (error) return showSaveError("Der Gruppenbeitritt", error); showNotice("Du bist der Gruppe beigetreten."); await loadAll(); }
  ...
  return <div className={`app layout-${profile?.profile_layout || "standard"}`}>
    ...
    <div className="dashboard-layout">
      ...
      <main className="modern-main"><div className="content-root">{notice && <div className="toast">{notice}</div>}
        ...
        {page === "community" && <><CommunityHub members={visibleMembers} events={communityEvents} ads={communityAds} photos={memberPhotos} profile={profile} deleteEvent={deleteCommunityEvent}/>{isAdmin(profile?.role) && <AdminCommunityTools members={members} isHeadAdmin={isHeadAdmin(profile?.role)} createEvent={createCommunityEvent} createAd={createCommunityAd} setBusinessAccount={setBusinessAccount}/>}</>}
        {page === "groups" && <GroupsPage groups={groups} members={members} profile={profile} user={user} transferRequests={groupOwnerChanges} onCreate={createGroup} onJoin={joinGroup} onLeave={leaveGroup} onEdit={editGroup} onTransfer={requestGroupOwnerChange} onReviewTransfer={reviewGroupOwnerChange} onOpen={setSelectedGroup}/>}
...
function PublicProfilePreview({ profile, photos, onBack }) { const privacy = profile?.privacy_settings || {}; const isPublic = (field) => (privacy[field] || "PUBLIC") === "PUBLIC"; const publicPhotos = photos.filter((photo) => photo.owner_id === profile?.id && photo.visibility !== "FRIENDS"); const background = profile?.profile_background?.startsWith("http") ? { backgroundImage: `linear-gradient(rgba(20,37,54,.68),rgba(20,37,54,.86)),url(${profile.profile_background})`, backgroundSize: "cover", backgroundPosition: "center" } : {}; return <section className="member-profile-page public-profile-preview"><button className="back-button" onClick={onBack}>← Zurück zum Bearbeiten</button><span className="eyebrow">VORSCHAU</span><p>So sehen Mitglieder, die nicht mit dir befreundet sind, dein Profil.</p><article className={`member-profile-hero ${roleClass(profile?.role)}`} style={background}><img src={profile?.avatar_url || DEFAULT_AVATAR} alt="Profilbild"/><div><span>{roleMark(profile?.role)} {roleLabel(profile?.role)}</span><h1>{getName(profile)}</h1>{isPublic("name") && <p>{[profile?.first_name, profile?.last_name].filter(Boolean).join(" ")}{isPublic("birth_date") && getAge(profile?.birth_date) !== null && ` · ${getAge(profile.birth_date)} Jahre`}</p>}{isPublic("bio") && profile?.bio && <p className="member-profile-bio">{profile.bio}</p>}{(profile?.role === "HEAD_ADMIN" || profile?.admin_responsibilities?.length) && <p className="admin-responsibilities">Zuständig für: {profile.role === "HEAD_ADMIN" ? (profile.head_admin_responsibilities || "Gesamtverantwortung, Sicherheit & Regeln") : profile.admin_responsibilities.join(" · ")}</p>}</div></article><section className="panel profile-visible-details"><span className="eyebrow">PROFILINFORMATIONEN</span>{isPublic("location") && profile?.location && <p>Ort: {profile.location}</p>}{isPublic("interests") && formatInterests(profile?.interests) && <p>Interessen: {formatInterests(profile.interests)}</p>}{isPublic("website") && profile?.website && <p>Webseite: {profile.website}</p>}{!isPublic("location") && !isPublic("interests") && !isPublic("website") && <p>Keine weiteren Informationen sind öffentlich sichtbar.</p>}</section>{publicPhotos.length > 0 && <PublicProfilePhotoFolder member={profile} photos={publicPhotos} canSeeFriends={false}/>}</section>; }

function ProfileTimeline({ visits, activities, members, onOpen }) { const memberFor = (id) => members.find((member) => member.id === id); const profileLink = (member, children) => member ? <button type="button" className="profile-activity-link" onClick={() => onOpen(member)}>{children}</button> : children; const roleName = (member) => member ? `${roleMark(member.role)} ${getName(member)}`.trim() : "Mitglied"; return <section className="profile-activity-dashboard"><article className="profile-timeline panel"><span className="eyebrow">PROFILBESUCHE</span><h2>Die letzten 10 Besuche</h2><div>{visits.slice(0,10).map((visit) => { const visitor = memberFor(visit.visitor_id); return <p key={`visit-${visit.id || visit.visited_at}`}>{profileLink(visitor, <><strong>{roleName(visitor)}</strong> hat dein Profil besucht</>)}<time>{new Date(visit.visited_at).toLocaleString("de-AT")}</time></p>; })}{!visits.length && <p>Noch keine Profilbesuche.</p>}</div></article><article className="profile-timeline panel"><span className="eyebrow">PROFIL-AKTUALISIERUNGEN</span><h2>Deine letzten 10 Änderungen</h2><div>{activities.slice(0,10).map((activity) => { const actor = memberFor(activity.actor_id); return <p key={activity.id}>{profileLink(actor, <><strong>{roleName(actor)}</strong> · {activity.activity_type}</>)}<time>{new Date(activity.created_at).toLocaleString("de-AT")}</time></p>; })}{!activities.length && <p>Noch keine Profil-Aktualisierungen.</p>}</div></article></section>; }
...
function AccountReview({ queue, members, onBack, onOpen }) { return <section className="account-review-page"><div className="page-heading"><div><span className="eyebrow">KONTOSCHUTZ</span><h1>Verifizierungen prüfen</h1><p>Hier erscheinen nur Profile, für die wegen eines Fake-Verdachts eine Verifizierung angefordert wurde.</p></div><button className="secondary-button" onClick={onBack}>← Zur Admin-Zentrale</button></div>{queue.length ? <div className="report-list">{queue.map((item) => <article key={`${item.user_id}-${item.review_reason}`} className="report-card"><strong>{item.nickname}</strong><p>{item.review_reason}</p><small>Frist: {new Date(item.registered_at).toLocaleString("de-AT")}</small><div className="content-manage-actions"><button className="secondary-button" onClick={() => { const member = members.find((entry) => entry.id === item.user_id); if (member) onOpen(member); }}>Profil und Rechte öffnen</button></div></article>)}</div> : <div className="empty-card">Keine Verifizierungen stehen derzeit aus.</div>}</section>; }

function AdminPanel({ members, memberEmails, profile, user, onOpen, updateMemberRole, toggleSuspension, setBusinessAccount, editingMember, setEditingMember, saveMemberData, adminTarget, loadPermissions, permissionDraft, setPermissionDraft, savePermissions, savingPermissions, openAccountReview }) {
...
function GroupsPage({ groups, members, profile, user, transferRequests, onCreate, onJoin, onLeave, onEdit, onTransfer, onReviewTransfer, onOpen }) {
  ...
  useEffect(() => { document.querySelectorAll(".groups-page .group-card").forEach((card, index) => { card.onclick = (event) => { if (event.target.closest("button,input,textarea,label")) return; const group = groups[index]; if (group) onOpen(group); }; }); }, [groups, onOpen]);
