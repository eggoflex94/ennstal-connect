...
function AdminPanel({ members, memberEmails, adminLog, profile, user, onOpen, updateMemberRole, toggleSuspension, setBusinessAccount, editingMember, setEditingMember, saveMemberData, adminTarget, loadPermissions, permissionDraft, setPermissionDraft, savePermissions, savingPermissions, openAccountReview }) {
  ...
  return <section className="admin-page"><div className="page-heading"><div><span className="eyebrow">VERWALTUNG</span><h1>Admin-Zentrale</h1><p>Global Admin und Community Admin werden klar unterschieden. Berechtigungen sind einzeln steuerbar.</p></div></div>
    ...
    {isHeadAdmin(profile?.role) && <section className="admin-log-panel panel"><span className="eyebrow">VERTRAULICH · NUR GLOBAL ADMIN</span><h2>Admin-Logbuch</h2><p>Protokollierte Verwaltungsaktionen mit ausführendem Konto, betroffenem Konto und Zeitpunkt.</p><div className="admin-log-list">{adminLog.map((entry) => { const actor = members.find((m) => m.id === entry.actor_id); const target = members.find((m) => m.id === entry.target_id); return <article className="admin-log-row" key={entry.id}><div><strong>{entry.action}</strong><span>Ausgeführt von: {getName(actor) || "System"}{target ? ` · Betroffen: ${getName(target)}` : ""}</span>{entry.details && Object.keys(entry.details).length > 0 && <small>{JSON.stringify(entry.details)}</small>}</div><time>{new Date(entry.created_at).toLocaleString("de-AT")}</time></article>; })}{!adminLog.length && <div className="empty-card">Noch keine Verwaltungsaktionen protokolliert.</div>}</div></section>}
  </section>;
}
