...
function AccountReview({ queue, members, onBack, onOpen, onReviewRegistration }) { return <section className="account-review-page"><div className="page-heading"><div><span className="eyebrow">KONTOSCHUTZ</span><h1>Konten prüfen</h1><p>Hier erscheinen noch nicht bestätigte E-Mail-Adressen sowie angeforderte Verifizierungen mit Frist. Das sind Hinweise zur Prüfung, kein automatischer Fake-Verdacht.</p></div><button className="secondary-button" onClick={onBack}>← Zur Admin-Zentrale</button></div>{queue.length ? <div className="report-list">{queue.map((item) => { const approval = item.review_reason === "Registrierung wartet auf Freigabe"; const verificationDue = String(item.review_reason || "").startsWith("Verifizierung angefordert"); return <article key={`${item.user_id}-${item.review_reason}`} className="report-card"><strong>{item.nickname}</strong><p>{item.review_reason}</p><small>{verificationDue ? "Frist: " : "Registriert: "}{new Date(item.registered_at).toLocaleString("de-AT")}</small><div className="content-manage-actions">{approval && <><button className="primary-button" onClick={() => onReviewRegistration(item.user_id, true)}>Freigeben</button><button className="danger-button" onClick={() => onReviewRegistration(item.user_id, false)}>Ablehnen</button></>}<button className="secondary-button" onClick={() => { const member = members.find((entry) => entry.id === item.user_id); if (member) onOpen(member); }}>Profil und Rechte öffnen</button></div></article>; })}</div> : <div className="empty-card">Keine Konten benötigen derzeit eine Prüfung.</div>}</section>; }
...
function MemberProfile({ member, friends, user, viewerProfile, friendship, back, onOpen, requestFriend, respond, removeFriend, blockUser, reportUser, warnMember, updateMemberRole, toggleSuspension, toggleTestAccount, setBusinessAccount, setForumModerator, setProfileVerification, loadPermissions, setMemberFeatureLock, openChat }) {
  ...
  useEffect(() => { const hero = document.querySelector(".member-profile-hero"); if (!hero || hero.querySelector(".admin-responsibilities") || (!isAdmin(member.role) && !member.forum_moderator)) return; const list = member.role === "HEAD_ADMIN" ? ["Gesamtverantwortung, Sicherheit & Regeln"] : [...(member.admin_responsibilities || []), ...(member.forum_moderator ? ["Forum-Moderation · Meldungen und respektvoller Austausch"] : [])]; if (!list.length) return; const box = document.createElement("small"); box.className = "admin-responsibilities"; box.textContent = `Zuständig für: ${list.join(" · ")}`; hero.querySelector("div")?.appendChild(box); }, [member]);
