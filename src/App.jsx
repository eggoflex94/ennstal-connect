...
export default function App() {
  ...
  async function deleteForumPost(post) { const mayModerate = post.scope === "COMMUNITY" && profile?.forum_moderator; if (post.author_id !== user?.id && !isHeadAdmin(profile?.role) && !mayModerate) return showNotice("Du kannst nur eigene Beiträge löschen."); if (!confirm(`Beitrag „${post.title}" wirklich löschen?`)) return; const { error } = await supabase.rpc("forum_delete_post", { p_post_id: post.id }); if (error) return showNotice(error.message); showNotice("Forumsbeitrag gelöscht."); await loadAll(); }
...
function MemberProfile({ member, friends, user, viewerProfile, friendship, back, onOpen, requestFriend, respond, removeFriend, blockUser, reportUser, warnMember, updateMemberRole, toggleSuspension, setBusinessAccount, setForumModerator, loadPermissions, setMemberFeatureLock, openChat }) {
  const incoming = friendship?.status === "PENDING" && friendship.receiver_id === user.id;
  const sent = friendship?.status === "PENDING" && friendship.requester_id === user.id;
  const accepted = friendship?.status === "ACCEPTED";
  ...
  useEffect(() => { const hero = document.querySelector(".member-profile-hero"); if (hero) { hero.classList.toggle("business-profile", member.account_badge === "BUSINESS"); if (member.profile_background?.startsWith("http")) { hero.style.backgroundImage = `linear-gradient(rgba(20,37,54,.68), rgba(20,37,54,.86)), url(${member.profile_background})`; hero.style.backgroundSize = "cover"; hero.style.backgroundPosition = "center"; } } const tools = document.querySelector(".member-admin-tools>div"); if (tools && isHeadAdmin(viewerProfile?.role) && !tools.querySelector(".profile-business-tool")) { const button = document.createElement("button"); button.className = "secondary-button profile-business-tool"; button.textContent = member.account_badge === "BUSINESS" ? "★ Unternehmenskonto entfernen" : "★ Unternehmenskonto"; button.onclick = () => setBusinessAccount(member.id, member.account_badge !== "BUSINESS"); tools.appendChild(button); } if (tools && isHeadAdmin(viewerProfile?.role) && member.role === "SUPPORTER" && !tools.querySelector(".forum-moderator-tool")) { const button = document.createElement("button"); button.className = "secondary-button forum-moderator-tool"; button.textContent = member.forum_moderator ? "★ Forum-Moderation entfernen" : "★ Zum Forum-Moderator ernennen"; button.onclick = () => setForumModerator(member, !member.forum_moderator); tools.appendChild(button); } }, [member, viewerProfile?.role, setBusinessAccount, setForumModerator]);
