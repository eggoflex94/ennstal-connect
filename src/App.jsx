...
/>}
        ...
        {page === "profile" && <section className="profile-page-layout"><Profile profile={profile} user={user} isHeadAdmin={isHeadAdmin} saveProfile={saveProfile} uploadProfileImage={uploadProfileImage} uploadProfileBackground={uploadProfileBackground} uploadProfileBioImage={uploadProfileBioImage} openPublicPreview={() => setPage("profile-preview")}/><ProfileSections member={profile} editable><MemberGroups member={profile} groups={groups} onOpen={(group)=>{setSelectedGroup(group);setPage("groups")}}/></ProfileSections><ProfileTimeline visits={profileVisits} activities={profileActivities} members={members} onOpen={openMember}/><ProfileWelcomeBadges badges={welcomeBadges}/><ProfilePhotoGallery photos={memberPhotos.filter((photo) => photo.owner_id === user.id)} likes={photoLikes} comments={photoComments} user={user} onUpload={uploadMemberPhoto} onLike={togglePhotoLike} onComment={addPhotoComment} onDelete={deleteMemberPhoto}/></section>}
...
function Profile({ profile, user, isHeadAdmin, saveProfile, uploadProfileImage, uploadProfileBioImage }) { const background = profile?.profile_background || "#1b1f26"; const isImage = background.startsWith("http"); const cardStyle = { backgroundColor: isImage ? "#1b1f26" : background, backgroundImage: "none" }; const bioStyle = { color: profile?.bio_color || "#f1f5f9" }; return <section><div className="my-area-layout"><div className={`my-profile-card ${profile?.account_badge === "BUSINESS" ? "business" : roleClass(profile?.role)}`} style={cardStyle}><img className="profile-role-star" src={profile?.role === "HEAD_ADMIN" || profile?.role === "ADMIN" ? "/role-star-red.svg" : profile?.role === "SUPPORTER" ? "/supporter-star.svg" : "/role-star-member.svg"} alt=""/><h1>{getName(profile)}</h1><div className={`profile-role-label ${roleClass(profile?.role)}`}>{profile?.role === "HEAD_ADMIN" ? "Hauptadmin · Betreiber" : profile?.account_badge === "BUSINESS" ? "Unternehmenskonto" : roleLabel(profile?.role)}</div><img className="my-avatar" src={profile?.avatar_url || DEFAULT_AVATAR} alt="Profil"/><h2>{[profile?.first_name, profile?.last_name].filter(Boolean).join(" ")}{getAge(profile?.birth_date) !== null && ` · ${getAge(profile.birth_date)} Jahre`}</h2>{profile?.bio_image_url && <img className="profile-bio-image" src={profile.bio_image_url} alt="Bild zum Über-mich-Bereich"/>}<p className={`profile-bio ${profile?.bio_font || "modern"} ${profile?.bio_size || "normal"}`} style={bioStyle}>{profile?.bio || "Noch kein Über-mich-Text."}</p></div><form className="panel profile-form profile-editor" onSubmit={saveProfile}>
...
function CommunityHub({ members, events, ads, photos, profile, profileUpdates, activeRegion, onDeleteAd }) {
