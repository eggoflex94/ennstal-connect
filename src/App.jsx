...
export default function App() {
  ...
  if (!user) return <div className="auth-page"><div className="text-logo">ENNSTAL CONNECT</div><Auth login={login} register={register}/><aside className="auth-online-rewards"><strong>⏱ Aktiv sein lohnt sich</strong><span>Mit deinen Online-Stunden schaltest du nach und nach besondere Profildesigns frei – vom Alpen-Layout bis zum Neon-Connect-Design ab 180 Stunden.</span><small>Admins, Supporter und Unternehmenskonten können alle Layouts sofort verwenden.</small></aside><button className="forgot-password-button" onClick={requestPasswordReset}>Passwort vergessen?</button>{notice && <div className="toast">{notice}</div>}</div>;
