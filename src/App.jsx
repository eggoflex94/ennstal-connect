...
function RoleStar({ member }) { const role=member?.role; const src=role==='HEAD_ADMIN'||role==='ADMIN'?'/role-star-red.svg':role==='SUPPORTER'?'/supporter-star.svg':member?.account_badge==='BUSINESS'?'/role-star-blue.svg':'/role-star-member.svg'; return <img className="ec-inline-role-star" src={src} alt="" aria-hidden="true"/>; }
...
/>}
        ...
        {page === "member-profile" && viewingMember && <MemberGroups member={viewingMember} groups={allGroups} onOpen={(group) => { setSelectedGroup(group); setPage("groups"); }}/>}
