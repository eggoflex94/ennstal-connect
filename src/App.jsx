...
export default function App() {
  ...
  useEffect(() => {
    const handleNavigation = (event) => {
      ...
      if (requested === "fake-accounts" || requested === "admin-log") {
        if (isHeadAdmin(profile?.role) && profile?.account_status === "ACTIVE") setPage(requested);
        return;
      }
      const pageMap = {
  ...
  useEffect(() => {
    ...
    if (page === "community" || page === "events") {
      ...
      eventCard?.querySelectorAll(".hub-row").forEach((row, index) => {
        ...
        if (entry.status !== "CANCELLED" && !row.querySelector(".event-rsvp-actions")) { const rsvp = document.createElement("div"); rsvp.className = "event-rsvp-actions"; const current = eventRsvps.find((item) => item.event_id === entry.id && item.user_id === user?.id)?.status; ["INTERESTED", "GOING"].forEach((status) => { const button = document.createElement("button"); button.className = current === status ? "primary-button" : "secondary-button"; button.textContent = status === "GOING" ? "✓ Ich komme" : "☆ Interessiert"; button.onclick = () => respondToCommunityEvent(entry, status); rsvp.appendChild(button); }); const share = document.createElement("button"); share.className = "secondary-button"; share.textContent = "↗ Auf meinem Profil teilen"; share.onclick = () => shareCommunityEvent(entry); rsvp.appendChild(share); row.appendChild(rsvp); }
  ...
  async function createCommunityAd(e) { e.preventDefault(); if (!isHeadAdmin(profile?.role)) return showNotice("Werbeflächen verwaltet nur der Hauptadmin."); if (!activeRegionId) return showNotice("Bitte zuerst eine Region auswählen."); const f = new FormData(e.currentTarget); const title = String(f.get("title") || "").trim(); if (title.length < 3) return showNotice("Bitte gib einen Namen für die Werbefläche ein."); let imageUrl = String(f.get("image_url") || "").trim() || null; try { const uploaded = await uploadContentImage(f.get("image"), "ads"); if (uploaded) imageUrl = uploaded; } catch (error) { return showSaveError("Das Werbebild", error); } const { error } = await supabase.rpc("ec_admin_create_regional_ad", { p_title: title, p_body: String(f.get("body") || "").trim(), p_link_url: String(f.get("link_url") || "").trim() || null, p_image_url: imageUrl, p_region: activeRegionId }); if (error) return showSaveError("Die Werbefläche", error); e.currentTarget.reset(); showNotice("Werbefläche veröffentlicht."); await loadAll(); }
  ...
  async function createForumPost(e, scope) { e.preventDefault(); if (scope === "COMMUNITY" && isFeatureLocked("FORUM_POSTING")) return showNotice("Deine Forums-Schreibfunktion ist derzeit vorübergehend gesperrt."); if (!activeRegionId) return showNotice("Bitte zuerst eine Region auswählen."); const form = new FormData(e.currentTarget); const payload = { scope, title: String(form.get("title") || "").trim(), content: String(form.get("content") || "").trim(), font_family: form.get("font_family") || "modern", font_size: form.get("font_size") || "normal", emphasis: form.get("emphasis") || "normal", is_ai_generated: !isAdmin(profile?.role) && form.get("is_ai_generated") === "on" }; if (payload.title.length < 3 || payload.content.length < 3) return showNotice("Bitte Überschrift und Beitrag ausfüllen."); const { error } = await supabase.rpc("ec_forum_create_regional_post", { p_scope: payload.scope, p_title: payload.title, p_content: payload.content, p_font_family: payload.font_family, p_font_size: payload.font_size, p_emphasis: payload.emphasis, p_region: activeRegionId, p_is_ai_generated: payload.is_ai_generated }); if (error) return showNotice(error.message); e.currentTarget.reset(); showNotice("Beitrag veröffentlicht."); await loadAll(); }
  ...
  async function createCommunityRequest(e) { e.preventDefault(); if (!activeRegionId) return showNotice("Bitte zuerst eine Region auswählen."); const form = new FormData(e.currentTarget); const title = String(form.get("title") || "").trim(); const content = String(form.get("content") || "").trim(); if (title.length < 5 || content.length < 10) return showNotice("Bitte einen Titel und eine kurze Beschreibung eingeben."); const { error } = await supabase.from("community_requests").insert({ author_id: user.id, category: form.get("category"), title, content, region_id: activeRegionId }); if (error) return showSaveError("Der Community-Aufruf", error); e.currentTarget.reset(); showNotice("Dein Aufruf ist jetzt für diese Region sichtbar."); await loadAll(); }
...
/>}
        ...
        {page === "profile" && <section className="profile-page-layout"><Profile profile={profile} user={user} isHeadAdmin={isHeadAdmin} saveProfile={saveProfile} uploadProfileImage={selectProfilePhotoForEdit} editProfileImage={editExistingProfilePhoto} uploadProfileBackground={selectProfileCoverForEdit} editProfileCover={() => setProfileCoverEditingExisting(true)} removeProfileCover={removeProfileCover} uploadProfileBioImage={uploadProfileBioImage} openPublicPreview={() => setPage("profile-preview")}/><HeadAdminSelfControls profile={profile} user={user} regions={regions} onChanged={loadAll} showNotice={showNotice}/><BusinessProfileManager profile={profile} user={user}/><ProfileSections member={profile} editable><MemberGroups member={profile} groups={allGroups} onOpen={(group)=>{setSelectedGroup(group);setPage("groups")}}/></ProfileSections><ProfileTimeline visits={profileVisits} activities={profileActivities} members={members} onOpen={openMember}/><ProfileWelcomeBadges badges={welcomeBadges}/><ProfilePhotoGallery photos={memberPhotos.filter((photo) => photo.owner_id === user.id)} likes={photoLikes} comments={photoComments} user={user} onUpload={uploadMemberPhoto} onLike={togglePhotoLike} onComment={addPhotoComment} onDelete={deleteMemberPhoto}/></section>}
        ...
        {page === "photos" && <EventPhotosPage user={user} profile={profile} members={members} regions={regions} activeRegion={activeRegion} showNotice={showNotice}/>} 
...
function Profile({ profile, user, isHeadAdmin, saveProfile, uploadProfileImage, editProfileImage, uploadProfileBackground, editProfileCover, removeProfileCover, uploadProfileBioImage }) { const background = profile?.profile_background || "#1b1f26"; const isImage = background.startsWith("http"); const business = profile?.account_badge === "BUSINESS"; return <section>{profile?.is_community_photographer && <div className="community-photographer-self-banner panel"><img src="/community-photographer-camera.svg" alt=""/><div><span className="eyebrow">COMMUNITY-FOTOGRAF</span><strong>{profile?.community_photographer_global ? "Für alle Regionen freigeschaltet" : "Regional freigeschaltet"}</strong><small>Eventfotos kannst du im Bereich „Fotos“ hochladen und verwalten.</small></div></div>}<div className="my-area-layout"><div className="profile-member-card-preview" aria-label="Vorschau deiner Mitgliederkarte"><MemberCardView member={profile} profile={profile} friendships={[]} interactive={false}/></div><form className="panel profile-form profile-editor" onSubmit={saveProfile}>
...
</form></div><ProfileShareQr profile={profile}/></section>; }
  ...
  return <div className="auth-welcome ec-auth-welcome">
    <section className="auth-intro ec-auth-intro">
      ...
      <div className="ec-auth-regions" aria-label="Verfügbare Regionen">
        ...
        <article><strong>Leoben – Bruck – Mürzzuschlag</strong><span>Regional verbunden in der Obersteiermark.</span></article>
        <article><strong>Salzkammergut</strong><span>Für Austausch rund um das Salzkammergut.</span></article>
      </div>
...
function CommunityHub({ members, events, ads, photos, profile, profileUpdates, activeRegion, onDeleteAd, onToggleEventFeatured }) {
