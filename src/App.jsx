  ...
  return <div className="app">
    ...
    <div className="dashboard-layout">
      ...
      <main className="modern-main"><div className="content-root">{notice && <div className="toast">{notice}</div>}
        ...
        {page === "profile" && <><Profile profile={profile} user={user} isHeadAdmin={isHeadAdmin} saveProfile={saveProfile} uploadProfileImage={uploadProfileImage} uploadProfileBackground={uploadProfileBackground} uploadProfileBioImage={uploadProfileBioImage}/><ProfilePhotoGallery photos={memberPhotos.filter((photo) => photo.owner_id === user.id)} onUpload={uploadMemberPhoto}/><ProfileTimeline visits={profileVisits} activities={profileActivities} members={members}/></>}
        ...
        {page === "member-profile" && viewingMember && isHeadAdmin(profile?.role) && viewingMember.role !== "HEAD_ADMIN" && <FeatureUnlocks member={viewingMember} setMemberFeatureLock={setMemberFeatureLock}/>}
...
function AdminPanel({ members, memberEmails, profile, user, onOpen, updateMemberRole, toggleSuspension, editingMember, setEditingMember, saveMemberData, adminTarget, loadPermissions, permissionDraft, setPermissionDraft, savePermissions, savingPermissions }) {
...
function MemberProfile({ member, friends, user, viewerProfile, friendship, back, onOpen, requestFriend, respond, removeFriend, blockUser, reportUser, warnMember, updateMemberRole, toggleSuspension, loadPermissions, setMemberFeatureLock, openChat }) {
  ...
  useEffect(() => { const hero = document.querySelector(".member-profile-hero"); if (hero && member.profile_background?.startsWith("http")) { hero.style.backgroundImage = `linear-gradient(rgba(20,37,54,.68), rgba(20,37,54,.86)), url(${member.profile_background})`; hero.style.backgroundSize = "cover"; hero.style.backgroundPosition = "center"; } }, [member]);
