...
export default function App() {
  ...
  async function toggleCommunityEventFeatured(event) {
    ...
    const { error } = await supabase.rpc("set_community_event_featured", { p_event_id: event.id, p_featured: next });
    if (error) return showNotice(error.message);
    setCommunityEvents((current) => current.map((entry) => entry.id === event.id ? { ...entry, is_featured: next, featured_at: next ? new Date().toISOString() : null, featured_by: next ? user.id : null } : entry));
...
/>}
        ...
        {page === "profile" && <section className="profile-page-layout"><Profile profile={profile} user={user} isHeadAdmin={isHeadAdmin} saveProfile={saveProfile} uploadProfileImage={uploadProfileImage} uploadProfileBackground={uploadProfileBackground} uploadProfileBioImage={uploadProfileBioImage} openPublicPreview={() => setPage("profile-preview")}/><ProfileSections member={profile} editable><MemberGroups member={profile} groups={allGroups} onOpen={(group)=>{setSelectedGroup(group);setPage("groups")}}/></ProfileSections><ProfileTimeline visits={profileVisits} activities={profileActivities} members={members} onOpen={openMember}/><ProfileWelcomeBadges badges={welcomeBadges}/><ProfilePhotoGallery photos={memberPhotos.filter((photo) => photo.owner_id === user.id)} likes={photoLikes} comments={photoComments} user={user} onUpload={uploadMemberPhoto} onLike={togglePhotoLike} onComment={addPhotoComment} onDelete={deleteMemberPhoto}/></section>}
