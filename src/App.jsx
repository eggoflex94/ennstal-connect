...
function Profile({ profile, user, isHeadAdmin, saveProfile, uploadProfileImage, uploadProfileBackground, uploadProfileBioImage }) { const background = profile?.profile_background || "#1b1f26"; const isImage = background.startsWith("http"); const cardStyle = isImage ? { "--profile-background-image": `linear-gradient(rgba(18,25,34,.42), rgba(18,25,34,.76)), url(${background})` } : { backgroundColor: background }; const bioStyle = { color: profile?.bio_color || "#f1f5f9" }; return <section><div className="my-area-layout"><div className={`my-profile-card ${profile?.account_badge === "BUSINESS" ? "business" : roleClass(profile?.role)}`} style={cardStyle}><div className="profile-role-mark">{profile?.role === "HEAD_ADMIN" ? "♛" : "★"}</div><img className="my-avatar" src={profile?.avatar_url || DEFAULT_AVATAR} alt="Profil"/><h1>{getName(profile)}</h1><div className={`profile-role-label ${roleClass(profile?.role)}`}>{profile?.account_badge === "BUSINESS" ? "Unternehmenskonto" : roleLabel(profile?.role)}</div>{isHeadAdmin(profile?.role) && <div className="head-admin-profile-badge">♛ GLOBAL ADMIN · VERANTWORTLICH</div>}<h2>{[profile?.first_name, profile?.last_name].filter(Boolean).join(" ")}{getAge(profile?.birth_date) !== null && ` · ${getAge(profile.birth_date)} Jahre`}</h2>{profile?.bio_image_url && <img className="profile-bio-image" src={profile.bio_image_url} alt="Bild zum Über-mich-Bereich"/>}<p className={`profile-bio ${profile?.bio_font || "modern"} ${profile?.bio_size || "normal"}`} style={bioStyle}>{profile?.bio || "Noch kein Über-mich-Text."}</p></div><form className="panel profile-form profile-editor" onSubmit={saveProfile}>
  ...
  <fieldset className="profile-editor-section">
    ...
    <input type="hidden" name="profile_background_image" value={isImage ? background : ""}/>
  </fieldset>
  <fieldset className="profile-editor-section">
    ...
    <div className="profile-editor-grid">
      ...
      <label className="profile-upload-field"><span>Hintergrundfoto</span><small>Wird hinter deinem Profil angezeigt</small><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(e) => e.target.files?.[0] && uploadProfileBackground(e.target.files[0])}/></label>
