...
function AccountReview({ queue, members, onBack, onOpen }) { return <section className="account-review-page"><div className="page-heading"><div><span className="eyebrow">KONTOSCHUTZ</span><h1>Konten prüfen</h1><p>Hier erscheinen noch nicht bestätigte E-Mail-Adressen sowie angeforderte Verifizierungen mit Frist. Das sind Hinweise zur Prüfung, kein automatischer Fake-Verdacht.</p></div><button className="secondary-button" onClick={onBack}>← Zur Admin-Zentrale</button></div>{queue.length ? <div className="report-list">{queue.map((item) => { const verificationDue = String(item.review_reason || "").startsWith("Verifizierung angefordert"); return <article key={`${item.user_id}-${item.review_reason}`} className="report-card"><strong>{item.nickname}</strong><p>{item.review_reason}</p><small>{verificationDue ? "Frist: " : "Registriert: "}{new Date(item.registered_at).toLocaleString("de-AT")}</small><div className="content-manage-actions"><button className="secondary-button" onClick={() => { const member = members.find((entry) => entry.id === item.user_id); if (member) onOpen(member); }}>Profil und Rechte öffnen</button></div></article>; })}</div> : <div className="empty-card">Keine Konten benötigen derzeit eine Prüfung.</div>}</section>; }
