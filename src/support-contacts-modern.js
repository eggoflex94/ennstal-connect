...
function roleLabel(role) {
  if (role === "HEAD_ADMIN") return "Betreiber";
...
function displayName(profile) {
  return profile.nickname || [profile.first_name, profile.last_name].filter(Boolean).join(" ") || "Community-Team";
}

function responsibilities(profile) {
  const role = normalizeRole(profile.role);
  const raw = role === "HEAD_ADMIN" ? profile.head_admin_responsibilities : profile.admin_responsibilities;
  const values = Array.isArray(raw) ? raw : String(raw || "").split(/[,;\n]+/);
  return values
    .map((entry) => String(entry || "").replace(/^Zuständig\s+für:\s*/i, "").trim())
    .filter(Boolean)
    .slice(0, 6);
}
...
function makeAvatar(profile) {
  if (profile.avatar_url) {
    ...
    image.className = "ec-team-avatar";
  ...
  fallback.className = "ec-team-avatar ec-team-avatar-fallback";
  fallback.textContent = displayName(profile).slice(0, 1).toUpperCase();
  return fallback;
}
...
function makePerson(profile) {
  ...
  card.className = `ec-team-person ec-role-${role.toLowerCase()}`;

  const top = document.createElement("div");
  top.className = "ec-team-person-top";
  top.appendChild(makeAvatar(profile));
  ...
  if (items.length) {
    ...
    details.className = "ec-team-details";
    const summary = document.createElement("summary");
    summary.textContent = "Zuständigkeiten";
...
function renderTeam(profiles) {
  ...
  if (!panel) {
    ...
    const oldPanel = host.querySelector(".ec-support-panel");
  ...
  title.textContent = "Support & Ansprechpartner";
  const copy = document.createElement("p");
  copy.textContent = "Für sämtliche Support-Anfragen kannst du den Betreiber per Privatnachricht kontaktieren oder dich an die unten angeführten Admins und Supporter wenden.";
  introText.append(title, copy);
  ...
  if (!profiles.length) {
    ...
    empty.className = "ec-team-empty";
    empty.textContent = "Derzeit sind keine Ansprechpartner eingetragen.";
    panel.appendChild(empty);
    return;
  }

  const groups = [
    ["HEAD_ADMIN", "Betreiber"],
    ["ADMIN", "Admins"],
    ["SUPPORTER", "Supporter"]
  ];

  groups.forEach(([role, label]) => {
    const members = profiles.filter((profile) => normalizeRole(profile.role) === role);
    if (!members.length) return;
    const group = document.createElement("div");
    group.className = "ec-team-group";
    const groupTitle = document.createElement("div");
    groupTitle.className = "ec-team-group-title";
    groupTitle.textContent = `${label} · ${members.length}`;
...
function signature(profiles) {
  return profiles.map((profile) => [
    ...
    profile.last_name,
    profile.avatar_url,
    JSON.stringify(profile.admin_responsibilities || null),
    profile.head_admin_responsibilities || ""
  ].join("|")).join("::");
}
...
async function refreshTeam() {
  ...
  try {
    const { data, error } = await supabase
      .from("profiles")
      .select("id,nickname,first_name,last_name,avatar_url,role,admin_responsibilities,head_admin_responsibilities")
      .in("role", ["HEAD_ADMIN", "ADMIN", "SUPPORTER"]);
    if (error) throw error;

    const profiles = (data || [])
      .filter((profile) => TEAM_ROLES.has(normalizeRole(profile.role)))
      .sort((a, b) => {
        const roleDiff = (ROLE_ORDER[normalizeRole(a.role)] ?? 9) - (ROLE_ORDER[normalizeRole(b.role)] ?? 9);
        return roleDiff || displayName(a).localeCompare(displayName(b), "de");
      });
