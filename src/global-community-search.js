...
function ensureStyle(){
  ...
  style.textContent=`
    ...
    .ec-global-search-help{padding:10px}.ec-global-search-help strong{display:block;color:#29445e;font-size:.72rem;margin-bottom:7px}.ec-global-search-help p{margin:0 0 8px;color:#738497;font-size:.64rem;line-height:1.45}.ec-global-search-help-list{display:flex;flex-wrap:wrap;gap:6px}.ec-global-search-help-list span{padding:5px 8px;border-radius:999px;background:#eef4f8;color:#36536e;font-size:.6rem;font-weight:850}
...
async function loadData(force=false){
  ...
  cache={members:parseDirectory(membersRes.data),groups:groupsRes.data||[],events:eventsRes.data||[],forum:forumRes.data||[],news:newsRes.data||[]};
  cacheAt=Date.now();
  return cache;
}
...
function score(text,q){
  ...
  if(hay===q)return 100;if(hay.startsWith(q))return 70;if(hay.includes(` ${q}`))return 55;if(hay.includes(q))return 35;return 0;
}
...
function ranked(data,q){
  ...
  return {members:take(data.members,memberHay),groups:take(data.groups,g=>`${g.name||''} ${g.description||''}`),events:take(data.events,e=>`${e.title||''} ${e.description||''} ${e.location||''}`),forum:take(data.forum,p=>`${p.title||''} ${p.content||''}`),news:take(data.news,n=>`${n.title||''} ${n.content||''}`)};
}

function openProfile(id){const ev=new CustomEvent('ec:open-profile',{detail:{profileId:id},cancelable:true});if(window.dispatchEvent(ev))window.location.assign(`/?profile=${encodeURIComponent(id)}`)}
function go(page){window.dispatchEvent(new CustomEvent('ec:navigate',{detail:{page}}));document.body.classList.remove('ec-dock-open')}
...
function resultButton(kind,row){
  ...
  if(kind==='members'){
    ...
    const [icon,label,page]=map[kind];const ico=document.createElement('span');ico.className='ec-global-search-type';ico.textContent=icon;
    const copy=document.createElement('span');const title=row.name||row.title||label;
...
function render(panel,results){
  panel.replaceChildren();const labels={members:'Mitglieder',groups:'Gruppen',events:'Events',forum:'Forum',news:'Neuigkeiten'};let total=0;
  Object.entries(results).forEach(([kind,rows])=>{if(!rows.length)return;total+=rows.length;const section=document.createElement('section');section.className='ec-global-search-section';const title=document.createElement('div');title.className='ec-global-search-section-title';title.textContent=labels[kind];section.appendChild(title);rows.forEach(row=>section.appendChild(resultButton(kind,row)));panel.appendChild(section)});
  if(!total){const empty=document.createElement('div');empty.className='ec-global-search-empty';empty.textContent='Keine passenden Mitglieder, Gruppen, Events, Forum-Beiträge oder Neuigkeiten gefunden.';panel.appendChild(empty)}
  activeIndex=-1;panel.hidden=false;positionPanel();
}

function showHelp(panel){
  panel.innerHTML='<div class="ec-global-search-help"><strong>Was kannst du durchsuchen?</strong><p>Die Suche durchsucht gleichzeitig die wichtigsten Bereiche von Ennstal Connect. Gib mindestens 2 Zeichen ein.</p><div class="ec-global-search-help-list"><span>Mitglieder</span><span>Gruppen</span><span>Events</span><span>Forum</span><span>Neuigkeiten</span></div></div>';
  panel.hidden=false;activeIndex=-1;positionPanel();
}
...
function positionPanel(){
  ...
  const r=root.getBoundingClientRect();const width=Math.min(520,window.innerWidth-20);let left=Math.min(r.left,window.innerWidth-width-10);left=Math.max(10,left);panel.style.left=`${left}px`;panel.style.top=`${Math.min(r.bottom+6,window.innerHeight-120)}px`;
}

async function doSearch(value){
  const q=norm(value);const panel=document.querySelector('.ec-global-search-panel');if(!panel)return;
  if(q.length<2){if(document.activeElement===document.querySelector('.ec-global-search input'))showHelp(panel);else panel.hidden=true;return}
  panel.innerHTML='<div class="ec-global-search-empty">Suche …</div>';panel.hidden=false;positionPanel();
  const data=await loadData();if(norm(document.querySelector('.ec-global-search input')?.value)!==q)return;render(panel,ranked(data,q));
}

function keyboard(event){
  const panel=document.querySelector('.ec-global-search-panel');if(!panel||panel.hidden)return;const items=[...panel.querySelectorAll('.ec-global-search-result')];
  if(event.key==='Escape'){panel.hidden=true;event.target.blur();return}if(!items.length)return;
  if(event.key==='ArrowDown'){event.preventDefault();activeIndex=(activeIndex+1)%items.length}else if(event.key==='ArrowUp'){event.preventDefault();activeIndex=(activeIndex-1+items.length)%items.length}else if(event.key==='Enter'&&activeIndex>=0){event.preventDefault();items[activeIndex].click();panel.hidden=true;return}else return;
  items.forEach((item,i)=>item.classList.toggle('is-active',i===activeIndex));items[activeIndex]?.scrollIntoView({block:'nearest'});
}

function mount(){
  const nav=document.querySelector('.ec-top-nav');if(!nav)return false;if(nav.querySelector('.ec-global-search'))return true;ensureStyle();
  const root=document.createElement('div');root.className='ec-global-search';root.title='Durchsucht Mitglieder, Gruppen, Events, Forum und Neuigkeiten';
  root.innerHTML='<span class="ec-global-search-icon" aria-hidden="true">⌕</span><input type="search" autocomplete="off" spellcheck="false" aria-label="Mitglieder, Gruppen, Events, Forum und Neuigkeiten durchsuchen" placeholder="Mitglieder, Gruppen, Events …"><button type="button" class="ec-global-search-clear" aria-label="Suche leeren">×</button>';
  ...
  input.addEventListener('focus',()=>void doSearch(input.value));input.addEventListener('keydown',keyboard);
  clear.addEventListener('click',()=>{input.value='';root.classList.remove('has-value');showHelp(panel);input.focus()});return true;
}

function schedule(retries=10){const run=(left)=>{if(mount()||left<=0)return;setTimeout(()=>run(left-1),180)};run(retries)}

document.addEventListener('click',(event)=>{const panel=document.querySelector('.ec-global-search-panel');if(!panel)return;if(!event.target.closest('.ec-global-search')&&!event.target.closest('.ec-global-search-panel'))panel.hidden=true});
window.addEventListener('resize',positionPanel,{passive:true});window.addEventListener('scroll',positionPanel,{passive:true});
