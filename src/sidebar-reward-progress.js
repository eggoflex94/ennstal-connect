...
const prestigePercent=(state)=>{
  if(!state)return 0;
  const score=Number(state.score||0);
  const next=state.prestige_next_score==null?null:Number(state.prestige_next_score);
  if(!next)return 100;
  const floor=state.prestige?prestigeFloor(state.prestige):300;
  return Math.max(0,Math.min(100,Math.round(((score-floor)/Math.max(1,next-floor))*100)));
};
...
function prestigeCopy(state){
  const score=Number(state.score||0);
  const prestige=state.prestige||'';
  const next=state.prestige_next_score==null?null:Number(state.prestige_next_score);
  if(score<300)return `Noch ${300-score} Punkte bis Prestige Bronze`;
  if(!next)return 'Prestige Platin erreicht';
  return `${prestige} · noch ${Math.max(0,next-score)} Punkte bis zur nächsten Stufe`;
}
...
function render(state){
  ...
  const nextCopy=next?`${Math.max(0,next-score)} Punkte bis ${next}`:'Community-Level abgeschlossen';
  const prestigeLabel=state.prestige?`Prestige ${state.prestige}`:'Prestige';
  card.innerHTML=`
    ...
    <div class="ec-dock-reward-row ec-dock-prestige-row"><span>${prestigeLabel}</span><em>${prestigeCopy(state)}</em></div>
    <div class="ec-dock-reward-bar ec-dock-reward-bar-prestige" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${prestigePercent(state)}"><i style="width:${prestigePercent(state)}%"></i></div>
    ...
    <div class="ec-dock-reward-unlocks">
      ...
      ${state.prestige?`<span class="is-prestige">★ ${state.prestige}</span>`:''}
    </div>`;
}
