...
function openPopupManager(){
  closeOverlay();go('admin');let tries=0;
  const timer=setInterval(()=>{tries+=1;const panel=document.querySelector('.ec-popup-admin-manager');if(panel){clearInterval(timer);panel.classList.add('ec-popup-admin-manager-focus');panel.scrollIntoView({behavior:'smooth',block:'start'});setTimeout(()=>panel.classList.remove('ec-popup-admin-manager-focus'),1800)}else if(tries>24){clearInterval(timer);window.dispatchEvent(new CustomEvent('ec:community-announcements-refresh'))}},120)
}
...
function makeUtilityButton(kind){const button=document.createElement('button');button.type='button';button.className=`ec-compact-menu-item ec-dashboard-utility-button ec-dashboard-${kind}-button`;const label=kind==='info'?'Info':'Popups verwalten';button.title=label;button.setAttribute('aria-label',label);button.innerHTML=`<span class="ec-compact-menu-icon">${kind==='info'?INFO_ICON:POPUP_ICON}</span><span class="ec-compact-menu-label">${label}</span>`;button.onclick=kind==='info'?openInfo:openPopupManager;return button}
function ensureUtilityButtons(){const dock=document.querySelector('.ec-right-dock'),grid=dock?.querySelector('.ec-compact-menu-grid');if(!dock||!grid)return;if(!grid.querySelector('.ec-dashboard-info-button'))grid.appendChild(makeUtilityButton('info'));const existing=grid.querySelector('.ec-dashboard-popups-button');if(canManagePopups&&!existing)grid.appendChild(makeUtilityButton('popups'));if(!canManagePopups&&existing)existing.remove()}

function rebuildVisitRow(row,id){
  const p=people.get(id);if(!p)return;
  const star=personStar(p);const time=row.querySelector('time')?.textContent||'';
  row.innerHTML=`<span class="ec-profile-visit-identity">${star?`<img class="ec-inline-role-star" src="${star}" alt="" aria-hidden="true">`:''}<strong>${esc(p.nickname||'Mitglied')}</strong><small>hat dein Profil besucht</small></span><time>${esc(time)}</time>`;
  row.dataset.profileId=id;row.classList.add('ec-profile-visit-row-clean');
}

function normalizeProfileVisits(){
  const panel=document.querySelector('.ec-dock-detail[data-panel="visits"]');if(!panel)return;
  const seen=new Set();
  [...panel.querySelectorAll('.ec-dock-detail-row')].forEach(row=>{
    const id=String(row.dataset.profileId||'');
    if(!id||id===viewerId||seen.has(id)){row.remove();return}
    seen.add(id);rebuildVisitRow(row,id);
  });
  const count=document.querySelector('.ec-dock-visits-count');if(count)count.textContent=String(panel.querySelectorAll('.ec-dock-detail-row').length)
}
...
async function loadRights(){
  ...
  try{
    ...
    const role=String(profile?.role||'').toUpperCase(),assigned=new Set((assignments||[]).filter(x=>x.active).map(x=>x.region_id));
