...
function rebuildVisitRow(row,id){const p=people.get(id);if(!p)return;const star=personStar(p);const time=row.querySelector('time')?.textContent||'';const dateTime=row.querySelector('time')?.getAttribute('datetime')||'';const signature=JSON.stringify([id,p.nickname,star,time,dateTime]);if(row.dataset.visitSignature===signature)return;row.dataset.visitSignature=signature;row.innerHTML=`<span class="ec-profile-visit-identity">${star?`<img class="ec-inline-role-star" src="${star}" alt="" aria-hidden="true">`:''}<strong>${esc(p.nickname||'Mitglied')}</strong><small>hat dein Profil besucht</small></span><time datetime="${esc(dateTime)}">${esc(time)}</time>`;row.dataset.profileId=id;row.classList.add('ec-profile-visit-row-clean')}
function normalizeProfileVisits(){const panel=document.querySelector('.ec-dock-detail[data-panel="visits"]');if(!panel)return;const seen=new Set();[...panel.querySelectorAll('.ec-dock-detail-row')].forEach(row=>{const id=String(row.dataset.profileId||'');if(!id||id===viewerId||seen.has(id)){row.remove();return}seen.add(id);rebuildVisitRow(row,id)});const count=document.querySelector('.ec-dock-visits-count');if(count){const value=String(panel.querySelectorAll('.ec-dock-detail-row').length);if(count.textContent!==value)count.textContent=value}}
