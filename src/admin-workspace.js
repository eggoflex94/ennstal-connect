...
async function open(){
  ...
  if(isHead){
    actionCard(grid,"⚠️","Fehlerzentrale","Automatisch erkannte Browser-, Server-, Netzwerk- und Layoutfehler öffnen.",()=>{close();window.dispatchEvent(new CustomEvent("ec:open-system-errors"))});
    ...
    const {data:bs}=await supabase.rpc("head_admin_backup_review_status");const last=bs?.[0]?.last_checked_at;backup.querySelector("small").textContent=last?`Zuletzt geprüft: ${new Date(last).toLocaleString("de-AT")} · Erstellt selbst kein Backup.`:"Noch keine Prüfung dokumentiert · Erstellt selbst kein Backup.";
  }
