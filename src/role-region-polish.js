...
function polishInlinePeople() { const region = activeRegion(); const regionId = region?.id || null; document.querySelectorAll('.group-moderator,.role-author,.content-author,.event-author,.forum-reply small,.forum-post-head p strong').forEach((node) => { if(node.closest('.ec-role-person,.ec-team-panel,.ec-moderator-identity-list,.ec-forum-moderator-panel'))return; const text = String(node.textContent || '').replace(/\s+/g, ' ').trim(); const profile = profiles.find((item) => text.includes(nickname(item))); if (!profile) return; const prefix = /^Erstellt von\b/i.test(text) ? 'Erstellt von ' : /^von\b/i.test(text) ? 'von ' : ''; const dateMatch = text.match(/(\d{1,2}\.\d{1,2}\.\d{2,4}[^]*)$/); const suffix = dateMatch ? ` · ${esc(dateMatch[1].replace(/^·\s*/, ''))}` : ''; const html = `${prefix ? `<span class="ec-role-prefix-inline">${esc(prefix.trim())}</span> ` : ''}${identityMarkup(profile, regionId)}${suffix}`; if (node.dataset.ecInlineSignature !== html || !node.querySelector('.ec-role-person')) { node.dataset.ecInlineSignature = html; node.innerHTML = html; } node.classList.add('ec-role-person-row'); }); }
