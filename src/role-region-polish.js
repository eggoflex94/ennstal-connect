...
function setHTMLIfChanged(node, html, signature) {
  if (!node) return;
  if (node.dataset.ecSignature === signature) return;
  node.dataset.ecSignature = signature;
  node.innerHTML = html;
}
...
function renderRegionalResponsibilities() {
  ...
  if (heading && heading.textContent !== 'Administration & Support') heading.textContent = 'Administration & Support';
  ...
  setHTMLIfChanged(panel, html, signature);
...
function renderSidebarIdentity(profile) {
  ...
  if (heading) {
    const html = `${star ? `<img class="ec-sidebar-role-star" src="${esc(star)}" alt="" aria-hidden="true">` : ''}<span>${esc(nickname(profile))}</span>`;
    if (heading.dataset.ecSignature !== html) {
...
function applyMemberRoleVisuals() {
  ...
  document.querySelectorAll('.member-card').forEach((card) => {
    ...
    card.dataset.roleFrame = theme;
    const star = starFor(profile, region?.id || null);
    card.querySelectorAll('.ec-card-badge-role,.member-role-star,.ec-inline-role-star').forEach((node) => {
      if (isPlainMember(profile)) node.style.display = 'none';
      else {
        node.style.removeProperty('display');
        const image = node.matches('img') ? node : node.querySelector('img');
        if (image && star && image.getAttribute('src') !== star) image.src = star;
      }
    });
  });
}
...
function isOwnGeneratedMutation(record) {
  const target = record.target?.nodeType === 1 ? record.target : record.target?.parentElement;
  return Boolean(target?.closest?.('.ec-team-panel,.ec-forum-moderator-panel,.group-moderators,.ec-dock-head'));
}

function boot() {
  void syncEverything(true);
  const observer = new MutationObserver((records) => {
    if (records.length && records.every(isOwnGeneratedMutation)) return;
    clearTimeout(window.__ecRoleRegionPolishTimer);
    window.__ecRoleRegionPolishTimer = setTimeout(() => void syncEverything(false), 160);
  });
  observer.observe(document.documentElement, { childList: true, subtree: true });
  window.addEventListener('ec:region-change', () => setTimeout(() => void syncEverything(true), 50));
  setInterval(() => { if (!document.hidden) void loadContext(false); }, 30000);
}
