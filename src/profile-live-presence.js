...
function lastSeenText(row) {
  ...
  try {
    return `Zuletzt aktiv: ${new Date(row.last_active_at).toLocaleString('de-AT', { dateStyle: 'short', timeStyle: 'short' })}`;
  } catch {
    return '';
  }
}
...
function ensureCard(page) {
  ...
  if (hero) hero.appendChild(card);
  else page.prepend(card);
  return card;
}

function hideLegacyPresence(page) {
  [...page.querySelectorAll('*')].forEach((node) => {
    if (!(node instanceof HTMLElement)) return;
    if (node.closest('.ec-live-profile-presence')) return;
    if (node.children.length) return;
    const text = String(node.textContent || '').trim();
    if (/^(Mobil online|Online|Offline)$/i.test(text) || /^Zuletzt aktiv:/i.test(text)) {
      const parent = node.parentElement;
      if (parent && parent !== page && !parent.classList.contains('member-profile-actions')) parent.classList.add('ec-legacy-presence-hidden');
    }
  });
}
...
function render(row) {
  ...
  card.querySelector('small').textContent = lastSeenText(row);
  card.querySelector('small').hidden = !lastSeenText(row);
}

async function load(profileId) {
  if (!profileId || !supabase) return;
  const { data, error } = await supabase.from('profiles')
    .select('id,is_online,last_active_at,presence_device,hide_online_status')
    .eq('id', profileId)
    .maybeSingle();
...
function subscribe(profileId) {
  ...
  if (!profileId) return;
  channel = supabase
    .channel(`ec-profile-presence-${profileId}`)
...
window.setInterval(() => { if (currentProfileId) void load(currentProfileId); }, 30_000);
