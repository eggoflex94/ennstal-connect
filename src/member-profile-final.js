...
async function functionLabel(target){
  ...
  if(data?.length){const names=data.map(a=>regions.find(r=>r.id===a.region_id)?.name).filter(Boolean);if(names.length)return`★ Regional Admin ${names.join(', ')}`}
...
async function build(root){
  if(root.dataset.ecMemberProfileFinal==='1')return;
  const hero=root.querySelector('.member-profile-hero');
  if(!hero)return;
  await context();
  const target=await targetFor(root);if(!target)return;
  root.dataset.ecMemberProfileFinal='1';root.dataset.ecTargetId=target.id;root.classList.add('ec-member-profile-final');
  const isFriend=await friendship(target.id);
  const avatar=hero.querySelector('img');
  const role=await functionLabel(target);
  ...
  const functionBox=document.createElement('div');functionBox.className='ec-mp-function';functionBox.innerHTML=`<span>Funktion</span><strong>${esc(role)}</strong>`;left.appendChild(functionBox);
  ...
  root.insertBefore(card,hero);hero.hidden=true;
  const admin=root.querySelector('.member-admin-tools');
  if(admin){
    admin.classList.add('ec-mp-admin-panel');admin.hidden=true;
    const toggle=document.createElement('button');toggle.type='button';toggle.className='ec-mp-admin-toggle';toggle.textContent='Admin Tools';functionBox.appendChild(toggle);
