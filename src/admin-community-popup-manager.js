...
const manageableRegions=()=>isHead()?state.regions:state.regions.filter(r=>managedRegionIds().has(r.id));
const canManage=()=>isHead()||manageableRegions().length>0;
...
function render(){
  ...
  panel.innerHTML=`<header class="ec-popup-admin-head"><div><span class="eyebrow">COMMUNITY-POPUPS</span><h2>Ankündigungen verwalten</h2><p>${isHead()?'Globale oder regionale Popups erstellen und freigeben.':'Du kannst Popups nur für deine freigegebene Region verwalten.'}</p></div><button type="button" class="ec-popup-new">+ Neues Popup</button></header><div class="ec-popup-admin-list">${items||'<p class="ec-popup-admin-empty">Noch keine verwaltbaren Popups vorhanden.</p>'}</div>`;
...
function scopeOptions(item){
  ...
  const global=isHead()?`<option value="" ${!item?.region_id?'selected':''}>Global · alle Regionen</option>`:'';
...
function openEditor(item){
  ...
  const defaultRegion=item?.region_id?regionForId(item.region_id)?.slug:(isHead()?'':manageableRegions()[0]?.slug||'');
