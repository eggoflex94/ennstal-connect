...
function applyRegionalDirectory(){if(!activeRegion||!profiles.length)return;document.documentElement.dataset.ecRegion=activeRegion.slug;const searching=Boolean(document.querySelector('.search-input')?.value.trim());document.querySelectorAll('.member-card').forEach(card=>{const p=profileForCard(card);if(!p)return;const theme=themeFor(p),label=roleLabel(p);card.dataset.memberId=p.id;card.dataset.homeRegionId=p.home_region_id||'';card.dataset.roleTheme=theme;card.dataset.effectiveRole=label;card.classList.remove('role-theme-admin','role-theme-supporter','role-theme-business','role-theme-member');card.classList.add(`role-theme-${theme}`);card.hidden=!searching&&p.home_region_id!==activeRegion.id;const badge=card.querySelector('.ec-card-badge-role');if(badge){badge.title=label;badge.setAttribute('aria-label',label)}const star=card.querySelector('.ec-card-badge-role-img');if(star)star.src=theme==='admin'?'/role-star-red.svg':theme==='supporter'?'/supporter-star.svg':theme==='business'?'/role-star-blue.svg':'/role-star-member.svg'});const grid=document.querySelector('.member-grid,.members-grid');if(grid){let bar=grid.previousElementSibling;if(!bar?.classList.contains('ec-region-context')){bar=document.createElement('div');bar.className='ec-region-context';grid.before(bar)}const content=`<div><span>Mitgliederbereich</span><strong>${esc(searching?'Suche in allen Regionen':activeRegion.name)}</strong><small>${searching?'Die Mitgliedersuche ist bewusst regionsübergreifend.':'Hier siehst du ausschließlich Mitglieder mit dieser Heimatregion.'}</small></div>`;if(bar.innerHTML!==content)bar.innerHTML=content}}
