...
function activeRegion() {
  ...
  const bySlug=cache.regions.find((region)=>String(region.slug||'')===slug);
  ...
  return cache.regions.find((region) => [region.name, region.short_name, region.slug].filter(Boolean).some((value) => String(value).trim().toLowerCase() === label)) || cache.regions[0] || null;
}
...
function personCard(member, regionId) {
  ...
  button.type = 'button';button.className = 'ec-home-fixed-person-head';button.title = `Profil von ${nameOf(member)} öffnen`;
  const avatar = document.createElement('img');avatar.className = 'ec-home-fixed-avatar';avatar.src = member?.avatar_url || '/community-default-avatar.png';avatar.alt = '';avatar.loading='eager';avatar.decoding='async';button.appendChild(avatar);
  const text = document.createElement('span');text.className = 'ec-home-fixed-person-text';
  const line = document.createElement('span');line.className = 'ec-home-fixed-name-line';const star = makeRoleIcon(member, 'normal');if (star) line.appendChild(star);
  const strong = document.createElement('strong');strong.textContent = nameOf(member);line.appendChild(strong);text.appendChild(line);
  const role = document.createElement('small');role.textContent = roleTitle(member, regionId);text.appendChild(role);button.appendChild(text);
  ...
  if (items.length) {const list = document.createElement('ul');items.forEach((item) => { const li = document.createElement('li'); li.textContent = item; list.appendChild(li); });card.appendChild(list)}
  return card;
}
...
function mergeProfiles(contacts, extras) {
  ...
  return (contacts || []).map((contact) => {const id = contact.user_id || contact.id;return { ...contact, ...(extraById.get(id) || {}), id, user_id: id }});
}
...
async function load() {
  ...
  try {
    ...
    cache = {profiles: mergeProfiles(contacts, extras),regions: regionsResult?.data || [],assignments: assignmentsResult?.error ? [] : (assignmentsResult?.data || [])};
    persistCache();render();
  } catch (error) {
...
window.addEventListener('ec:region-change', (event) => {activeRegionHint = event.detail || null;render();setTimeout(() => void load(), 0)});
