...
async function renderHome(){
  ...
  const [{data:events},{data:requests},{data:businesses},{data:listings},{data:newMembers},{data:groups},{data:memberships},{data:friendships}]=await Promise.all([
    supabase.from('community_events').select('id,title,event_at,location,status,region_id,is_featured,featured_color').eq('region_id',ctx.region.id).gte('event_at',now).order('event_at',{ascending:true}).limit(6),
...
async function renderOwnBusinessTools(){
  ...
  const [{data:listings},{data:ownEvents}]=await Promise.all([
    ...
    supabase.from('community_events').select('id,title,event_at,location,status,region_id,created_by,is_featured,featured_color').eq('created_by',ctx.user.id).order('event_at',{ascending:false}).limit(12)
  ]);
