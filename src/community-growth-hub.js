...
function eventCard(event){
  return `<button type="button" class="ec-growth-item ec-clickable ${event.is_featured?`is-featured featured-${event.featured_color||'gold'}`:''}" data-growth-nav="events">${event.is_ai_generated?'<span class="ai-content-badge">✦ KI-Inhalt</span>':''}${event.is_featured?'<span class="ec-growth-featured">★ Hervorgehoben</span>':''}<span class="ec-growth-tag">TERMIN</span><strong>${esc(event.title)}</strong><span>${esc(fmt(event.event_at))}${event.location?` · ${esc(event.location)}`:''}</span></button>`;
}
...
function listingCard(listing){
  return `<button type="button" class="ec-growth-item ec-clickable" data-growth-profile="${esc(listing.owner_id)}">${listing.is_ai_generated?'<span class="ai-content-badge">✦ KI-Inhalt</span>':''}<span class="ec-growth-tag">${esc(LISTING_LABELS[listing.listing_type]||'Angebot')}</span><strong>${esc(listing.title)}</strong><span>${esc(String(listing.body||'').slice(0,120))}</span></button>`;
}
...
async function renderHome(){
  ...
  const [{data:events},{data:requests},{data:businesses},{data:listings},{data:newMembers},{data:groups},{data:memberships},{data:friendships}]=await Promise.all([
    supabase.from('community_events').select('id,title,event_at,location,status,region_id,is_featured,featured_color,is_ai_generated').eq('region_id',ctx.region.id).gte('event_at',now).order('event_at',{ascending:true}).limit(6),
    ...
    supabase.from('business_listings').select('id,owner_id,region_id,listing_type,title,body,link_url,valid_until,is_active,is_ai_generated,created_at').eq('is_active',true).or(`region_id.eq.${ctx.region.id},region_id.is.null`).order('created_at',{ascending:false}).limit(10),
...
function businessPanelMarkup(member,listings=[]){
  ...
  const items=listings.length?`<div class="ec-business-listings"><h3>Aktuelle Angebote & Stellen</h3>${listings.map((item)=>`<article>${item.is_ai_generated?'<span class="ai-content-badge">✦ KI-Inhalt</span>':''}<span>${esc(LISTING_LABELS[item.listing_type]||'Angebot')}</span><strong>${esc(item.title)}</strong><p>${esc(item.body)}</p>${item.link_url?`<a href="${esc(item.link_url)}" target="_blank" rel="noreferrer">Mehr erfahren ↗</a>`:''}</article>`).join('')}</div>`:'';
...
async function renderBusinessProfile(){
  ...
  const [{data:member},{data:listings}]=await Promise.all([
    ...
    supabase.from('business_listings').select('id,owner_id,listing_type,title,body,link_url,valid_until,is_active,is_ai_generated,created_at').eq('owner_id',id).eq('is_active',true).order('created_at',{ascending:false})
  ]);
...
async function renderOwnBusinessTools(){
  ...
  const [{data:listings},{data:ownEvents}]=await Promise.all([
    ...
    supabase.from('community_events').select('id,title,event_at,location,status,region_id,created_by,is_featured,featured_color,is_ai_generated').eq('created_by',ctx.user.id).order('event_at',{ascending:false}).limit(12)
  ]);
  ...
  shell.querySelector('.ec-business-event-form').onsubmit=async(event)=>{
    ...
    const {error}=await supabase.from('community_events').insert({
      ...
      region_id:regionId,
      status:'ACTIVE',
      is_ai_generated:form.get('is_ai_generated')==='on'
    });
    if(error)return notice(error.message);
  ...
  shell.querySelectorAll('[data-feature-business-event]').forEach((button)=>button.onclick=async()=>{
    const featured=button.dataset.featured==='true';
    button.disabled=true;
    const color=!featured?(window.prompt('Farbe: gold, blue, green, red, purple oder orange','gold')||'gold').trim().toLowerCase():'gold';
    const {error}=await supabase.rpc('set_community_event_featured',{p_event_id:button.dataset.featureBusinessEvent,p_featured:!featured,p_color:color});
    if(error){button.disabled=false;return notice(error.message);}
    notice(featured?'Hervorhebung entfernt.':'Veranstaltung wurde hervorgehoben.');
    shell.remove();
    schedule(20);
  });
  ...
  shell.querySelector('.ec-business-listing-form').onsubmit=async(event)=>{
    ...
    const payload={owner_id:ctx.user.id,region_id:ctx.region?.id||ctx.profile.home_region_id||null,listing_type:listingType,title:String(form.get('title')||'').trim(),body:String(form.get('body')||'').trim(),link_url:String(form.get('link_url')||'').trim()||null,valid_until:valid?new Date(valid).toISOString():null,is_active:true,is_ai_generated:form.get('is_ai_generated')==='on'};
