...
function cleanDedicatedPersonBadges(){
  ...
  document.querySelectorAll(selectors).forEach(el=>{
    if(el.dataset.ecIdentityPolished==='true'||inGeneratedScope(el))return;
...
function cleanupAccidentalStarRuns(){
  document.querySelectorAll('.ec-moderator-identity-list,.group-moderators,.ec-team-panel').forEach(scope=>{
    const seen=new Set();
    [...scope.querySelectorAll('.ec-role-person')].forEach(person=>{
      const id=person.dataset.profileId||person.textContent.trim();
      if(seen.has(id))person.remove();else seen.add(id);
    });
    scope.querySelectorAll('.ec-global-role-identity').forEach(node=>node.remove());
  });
}

function polish(){
  if(!profiles.length)return;
  cleanupAccidentalStarRuns();
...
async function boot(){
  await loadProfiles();
  polish();
  const obs=new MutationObserver(()=>{clearTimeout(window.__ecGlobalRolePolish);window.__ecGlobalRolePolish=setTimeout(polish,90)});
