...
function ensureStyle(){
  ...
  style.textContent=`
    ...
    .ec-onboarding-v2-starter{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0 0 16px}.ec-onboarding-v2-starter article{padding:10px;border:1px solid #dfe8ef;border-radius:12px;background:#f9fcff}.ec-onboarding-v2-starter b{display:block;color:#0b6dcc;font-size:.68rem;margin-bottom:3px}.ec-onboarding-v2-starter span{display:block;color:#697d8d;font-size:.62rem;line-height:1.35}
    ...
    @media(max-width:620px){.ec-onboarding-v2-box{border-radius:16px}.ec-onboarding-v2-head,.ec-onboarding-v2-body,.ec-onboarding-v2-actions{padding-left:16px;padding-right:16px}.ec-onboarding-v2-options,.ec-onboarding-v2-starter{grid-template-columns:1fr}}
  `;
...
async function showWizard(user, profile, regions){
  ...
  box.innerHTML=`<header class="ec-onboarding-v2-head"><span class="eyebrow">WILLKOMMEN BEI ENNSTAL CONNECT</span><h2 id="ec-onboarding-v2-title">3 Dinge für deinen perfekten Start</h2><p>Region festlegen, Interessen auswählen und dein Profil kurz persönlich machen. Danach zeigen wir dir direkt passende Menschen, Gruppen und Veranstaltungen.</p><div class="ec-onboarding-v2-progress">${[0,1,2,3].map(i=>`<span data-p="${i}"></span>`).join('')}</div></header><div class="ec-onboarding-v2-body"><div class="ec-onboarding-v2-step" data-step="0"><div class="ec-onboarding-v2-starter"><article><b>1 · Region</b><span>Relevante Inhalte zuerst sehen</span></article><article><b>2 · Interessen</b><span>Passendere Empfehlungen bekommen</span></article><article><b>3 · Profil</b><span>Leichter von anderen gefunden werden</span></article></div><h3>1. Heimatregion bestätigen</h3><p>Deine Heimatregion bestimmt, welche lokalen Inhalte du zuerst siehst.</p><div class="ec-onboarding-v2-options">${regions.map(r=>`<label class="ec-onboarding-v2-option"><input type="radio" name="region" value="${r.slug}" ${r.id===profile.home_region_id?'checked':''}><span><strong>${r.name}</strong><small>${r.description||'Regionale Community, Termine und Ansprechpartner'}</small></span></label>`).join('')}</div></div><div class="ec-onboarding-v2-step" data-step="1"><h3>2. Was interessiert dich?</h3><p>Wähle ein paar Themen aus. So werden Mitglieder, Gruppen und Events für dich relevanter.</p><div class="ec-onboarding-v2-options">${interests.map(i=>`<label class="ec-onboarding-v2-option"><input type="checkbox" value="${i}" ${String(profile.interests||'').toLowerCase().includes(i.toLowerCase())?'checked':''}><span><strong>${i}</strong><small>Für persönlichere Empfehlungen</small></span></label>`).join('')}</div></div><div class="ec-onboarding-v2-step" data-step="2"><h3>3. Mach dein Profil persönlich</h3><p>Ein Spitzname und ein kurzer Satz reichen schon, damit andere leichter Gemeinsamkeiten entdecken.</p><label class="ec-onboarding-v2-field"><span>Spitzname</span><input name="nickname" maxlength="50"></label><label class="ec-onboarding-v2-field"><span>Kurz über dich</span><textarea name="bio" maxlength="1000" placeholder="Zum Beispiel: Wandern, Musik und regionale Veranstaltungen"></textarea></label></div><div class="ec-onboarding-v2-step" data-step="3"><h3>Dein persönlicher Start</h3><p>Fertig – hier sind erste Vorschläge passend zu deinen Angaben.</p><div class="ec-onboarding-v2-recs"></div></div><div class="ec-onboarding-v2-error" hidden></div></div><footer class="ec-onboarding-v2-actions"><button type="button" class="ec-onboarding-v2-skip">Später</button><button type="button" class="ec-onboarding-v2-back">Zurück</button><button type="button" class="ec-onboarding-v2-next">Weiter</button></footer>`;
  ...
  const paint=()=>{box.querySelectorAll('.ec-onboarding-v2-step').forEach((n,i)=>n.classList.toggle('is-active',i===step));box.querySelectorAll('[data-p]').forEach((n,i)=>n.classList.toggle('is-active',i<=step));back.style.visibility=step===0?'hidden':'visible';next.textContent=step===3?'Ennstal Heute öffnen':'Weiter'};
