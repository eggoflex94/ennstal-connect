...
const dom = new JSDOM('<main class="admin-page"></main>',{runScripts:'outside-only',url:'https://example.test'});
const w=dom.window;
...
await wait();
let panel=w.document.querySelector('.ec-fake-admin');
...
w.document.querySelector('.admin-page').remove();
w.document.body.innerHTML='<main class="admin-page"></main>'; await wait(); await wait();
