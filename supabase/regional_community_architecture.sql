...
where p.home_region_id is null and r.slug='ennstal';
...
create policy regional_admin_assignments_read on public.regional_admin_assignments
for select using (auth.uid() is not null);

create or replace function public.ec_role_text(p_user uuid default auth.uid())
returns text
language sql
stable
security definer
...
as $$
  select upper(coalesce(role::text,'MEMBER')) from public.profiles where id=p_user;
$$;

create or replace function public.ec_is_head_admin_user(p_user uuid default auth.uid())
returns boolean
language sql
stable
security definer
set search_path=public
as $$
  select coalesce(public.ec_role_text(p_user)='HEAD_ADMIN',false);
$$;

create or replace function public.ec_is_global_admin_user(p_user uuid default auth.uid())
returns boolean
language sql
stable
security definer
set search_path=public
as $$
  select coalesce(public.ec_role_text(p_user) in ('HEAD_ADMIN','ADMIN'),false);
$$;

create or replace function public.ec_is_regional_admin(p_region uuid, p_user uuid default auth.uid())
returns boolean
language sql
stable
security definer
...
as $$
  select exists(
    select 1 from public.regional_admin_assignments a
    where a.user_id=p_user and a.region_id=p_region and a.active
  );
$$;
...
create or replace function public.ec_effective_role_for(p_user uuid, p_region uuid)
returns text
language plpgsql
stable
security definer
...
create or replace function public.ec_set_regional_admin(p_target uuid, p_region_slug text, p_enabled boolean)
returns void
language plpgsql
security definer
set search_path=public
...
begin
  ...
  if p_enabled then
    ...
    on conflict(user_id,region_id) do update set active=true,granted_by=excluded.granted_by,updated_at=now();
    update public.profiles set role='SUPPORTER' where id=p_target and upper(role::text)='MEMBER';
  else
    update public.regional_admin_assignments set active=false,updated_at=now() where user_id=p_target and region_id=region_uuid;
  end if;
end;
$$;

create or replace function public.ec_set_global_admin(p_target uuid, p_enabled boolean)
returns void
language plpgsql
security definer
set search_path=public
...
begin
  ...
  if p_enabled then
    ...
    if exists(select 1 from public.regional_admin_assignments where user_id=p_target and active) then
      update public.profiles set role='SUPPORTER' where id=p_target;
    else
      update public.profiles set role='MEMBER' where id=p_target;
    end if;
  end if;
end;
$$;

create or replace function public.ec_change_home_region(p_region_slug text)
returns uuid
language plpgsql
security definer
set search_path=public
...
begin
  ...
  if last_change is not null and last_change > now() - interval '30 days' and not public.ec_is_head_admin_user(auth.uid()) then
    raise exception 'Die Heimatregion kann nur alle 30 Tage geändert werden.';
  end if;
...
create or replace function public.ec_head_set_home_region(p_target uuid, p_region_slug text)
returns uuid
language plpgsql
security definer
set search_path=public
...
create or replace view public.ec_member_directory as
select
  p.*,
  r.slug as home_region_slug,
  r.name as home_region_name,
  r.short_name as home_region_short_name
from public.profiles p
left join public.regions r on r.id=p.home_region_id;
...
-- Region ownership columns for region-bound community areas. Existing data stays in Ennstal.
do $$
...
begin
  ...
  if to_regclass('public.forum_posts') is not null then
    execute 'alter table public.forum_posts add column if not exists region_id uuid references public.regions(id)';
    execute format('update public.forum_posts set region_id=%L where region_id is null',ennstal_id);
    execute 'create index if not exists forum_posts_region_idx on public.forum_posts(region_id)';
  end if;
  if to_regclass('public.community_groups') is not null then
    execute 'alter table public.community_groups add column if not exists region_id uuid references public.regions(id)';
    execute format('update public.community_groups set region_id=%L where region_id is null',ennstal_id);
    execute 'create index if not exists community_groups_region_idx on public.community_groups(region_id)';
  end if;
  if to_regclass('public.news') is not null then
    execute 'alter table public.news add column if not exists region_id uuid references public.regions(id)';
    execute format('update public.news set region_id=%L where region_id is null',ennstal_id);
    execute 'create index if not exists news_region_idx on public.news(region_id)';
  end if;
  if to_regclass('public.community_events') is not null then
    execute 'alter table public.community_events add column if not exists region_id uuid references public.regions(id)';
    execute format('update public.community_events set region_id=%L where region_id is null',ennstal_id);
    execute 'create index if not exists community_events_region_idx on public.community_events(region_id)';
  end if;
  if to_regclass('public.community_ads') is not null then
    execute 'alter table public.community_ads add column if not exists region_id uuid references public.regions(id)';
    execute format('update public.community_ads set region_id=%L where region_id is null',ennstal_id);
    execute 'create index if not exists community_ads_region_idx on public.community_ads(region_id)';
  end if;
end $$;
