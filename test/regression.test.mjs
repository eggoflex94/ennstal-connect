...
test("native member card renders role theme, identity and separated age/presence", async()=>{const card=await source("src/MemberCardView.jsx");assert.match(card,/data-member-id=\{member\.id\}/);assert.match(card,/data-home-region-id=\{member\.home_region_id/);assert.match(card,/data-role-theme=\{presentation\.theme\}/);for(const x of ['role-star-red.svg','supporter-star.svg','role-star-blue.svg'])assert.match(card,new RegExp(x.replace('.','\\.')));assert.doesNotMatch(card,/role-star-member\.svg/);assert.match(card,/presentation\.star &&/);assert.match(card,/className="ec-member-realname"/);assert.match(card,/className="ec-member-age"/);assert.match(card,/isAdminCard && member\.last_active_at/);assert.match(card,/loading="lazy"/);assert.match(card,/tabIndex=\{0\}/);});
