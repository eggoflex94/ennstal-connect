...
const context={module:{exports:{}},exports:{},React};vm.runInNewContext(code,context);
...
test('keyboard on message button does not also open member profile',()=>{
 ...
 node.props.onKeyDown({key:'Enter',target:card,currentTarget:card,preventDefault(){}});assert.equal(opened,1);
});
test('private online status does not render last activity',()=>{
 const html=renderToStaticMarkup(React.createElement(Card,{member:{id:'a',nickname:'Test',hide_online_status:true,last_active_at:'2026-09-07T08:00:00Z'},profile:{id:'b'},onOpen(){},onMessage(){}}));
